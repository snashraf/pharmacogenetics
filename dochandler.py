...
class DocHandler:
	...
	def AnnOverview(self):
			...
			for (did, name) in lst:
				...
				self.sql.execute("select terms from chemicals where did = ?", (did,))
				...
				self.sql.execute("select a.advice, g.symbol, a.loe from annotations a join variants v on a.varid=v.varid join genes g on v.gid=g.gid where a.did = ? order by loe asc", (did,))
				
				advices = self.sql.fetchall()

				for (advice, symbol,loe) in advices:
					
					print "|| GENE: %s LevelOfEvidence: %s ||" % (symbol,loe), advice,"\n"
