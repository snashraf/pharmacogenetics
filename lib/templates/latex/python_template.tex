...
{%- for pair in json.pairs | sort(attribute="drugname") -%}

  {%- if pair.guideline.guid != "" -%}
    ...
    {% endif %}
      ...
      {% if pair.drugname != "curdrug" -%}

        \subsection{ {{pair.drugname}} }
        {% set curdrug = "pair.drugname" %}
      ...
      \textbf{ {{pair.guideline.summary | replace("#", "\#") | replace("_", "-") | replace("%", "\%")}} }
      {{pair.guideline.tex  | replace("#", "\#") | replace("_", "-") | replace("%", "\%")}}
      \end{tabularx}
      \end{center}
...
{% if pair.annotations['1A'] | length > 0 or
 pair.annotations['1B'] | length > 0 or
  pair.annotations['2A'] | length > 0  or
   pair.annotations['2B'] | length > 0 %}
\subsubsection{ {{pair.genesymbol}} }
\subsubsection{ {{pair.genename}} }

\subsection{ {{pair.drugname}} }
{{pair.drugdesc}}

\begin{center}
{% for ann in pair.annotations['1A'] -%}
\textbf{\colorbox{red} {Class 1A}} \textbf{ {{ann.rsid}} } \textit{ {{ann.patAllele}} }
{{ann.phenotype}}
{%- endfor %}
{% for ann in pair.annotations['1B'] -%}
\textbf{\colorbox{orange} {Class 1B}} \textbf{ {{ann.rsid}} } \textit{ {{ann.patAllele}} }
{{ann.phenotype}}
{%- endfor %}
{% for ann in pair.annotations['2A'] -%}
\textbf{\colorbox{yellow} {Class 2A}} \textbf{ {{ann.rsid}} } \textit{ {{ann.patAllele}} }
{{ann.phenotype}}
{%- endfor %}
{% for ann in pair.annotations['2B'] -%}
