...
# ---------------------------------- calculate old style --------------------------------------
					...
					for i, alleleScore in enumerate([float(s1), float(s2)]):
						...
						if (alleleScore >= 1.0 and hLen > curLen )  and starname in options:
				...
				for i, (gene, allele) in enumerate(genotype.items()):
					...
					allele_string_NEW= "/".join(allelesNEW)
					allele_string_OLD = "/".join(allelesOLD)

					genotypeNEW.append(":".join([gene, allele_string_NEW]))
					genotypeOLD.append(":".join([gene, allele_string_OLD]))
